...
function DataStore(dapper) {
  ...
  self.populate = function(tree, callback) {
    ...
    self.store = new self.providers[dapper.config.datastore.provider](dapper, store);

    self.store.load(function() {
      // Create initial models
      for (let organization in store.organizations) {
        organization = dapper.models.organization(organization, dapper.tree);
      }

      for (let group in store.groups) {
        group = dapper.models.group(group, dapper.tree);
      }

      for (let user in store.users) {
        user = dapper.models.user(user, dapper.tree);
      }
