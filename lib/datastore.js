...
function DataStore (dapper) {
  ...
  self.rebuild = () => {
    const tree = dapper.models.tree();
    Object.assign(tree.models, dapper.tree.models);
    dapper.tree = tree;
    self.build();
  };
  ...
  self.populate = function(tree, callback) {
    ...
    self.log.info(`${ self.provider.name } initializing...`);
    return self.provider.load((error) => {
      ...
      self.log.info(`${ self.provider.name } ready.`);
      return callback();
    });
  };
