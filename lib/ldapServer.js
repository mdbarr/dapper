...
function LDAPServer(dapper) {
  ...
  self.authorize = function(req, res, next) {
    ...
    if (dapper.tree.ldap.dns.has(dn)) {
      ...
      if (req instanceof ldap.BindRequest) {
        if (!req.credentials || !model || !model.password || !model.permissions.bind) {
          return next(new ldap.InvalidCredentialsError());
        }

        const mfaRequired = dapper.util.mfaRequired(dn, model);

        return dapper.auth.authenticate(req.credentials, model, mfaRequired, (authenticated) => {
          if (!authenticated) {
            return next(new ldap.InvalidCredentialsError());
          }
  ...
  self.search = function(req, res, next) {
    ...
    if (req.scope === 'base') {
      ...
      const seen = new Set();
      dapper.tree.ldap.dns.forEach((object, key) => {
  ...
  self.setBindings = function() {
    dapper.tree.ldap.binds.forEach((model, binding) => {
  ...
  self.boot = function(callback) {
    ...
    if (dapper.config.ldap.port === 'auto') {
      if (dapper.config.ldap.cert && dapper.config.ldap.key) {
