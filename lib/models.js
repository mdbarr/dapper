...
function Models(dapper) {
  ...
  self.ldap.organization = function(organization, tree = dapper.tree) {
    ...
    const o = `o=${ organization.name.toLowerCase() }`;

    const dns = new Set();
    const binds = new Set();

    model.dn = dapper.util.toDN([ o ], dns, binds);

    for (const dc in tree.ldap.domains) {
      model.dn = dapper.util.preferredDN(model.dn, dapper.util.toDN([ o, dc ], dns, binds));
    }
    ...
    model.dns.forEach(bind => tree.ldap.binds.set(bind, model));
      ...
      for (let o of orgs) {
        ...
        for (const key of dapper.config.users.keys) {
          ...
          if (key === dapper.config.users.primaryKey) {
            model.dn = dapper.util.preferredDN(model.dn, dapper.util.toDN([ value, ou, o, dc ], dn, bind));
          } else {
