...
function Models(dapper) {
  ...
  self.ldapGroup = function(group, tree = dapper.tree) {
    const model = {
      id: group.id,
      object: 'ldapGroup',

      dn: `cn=${ group.name }, ou=${ dapper.config.groups.ou }`,
      attributes: {
  ...
  self.ldapUser = function(user, tree = dapper.tree) {
    const model = {
      object: 'ldapUser',
      objectClass: [
        'top',
    ...
    tree.ldap.users[model.primaryDN] = model;
    tree.ldap.index[model.id] = model;

    tree.ldap.dns.set(model.primaryDN, model);
